  ...
  validate (done) {
    ...
    Object.keys(props.params).forEach((paramName) => {
      ...
      if (param.required && !this.params[paramName]) {
        return done(new ResourceError(`Missing required resource parameter '${paramName}'`))
      }
      if (this.params[paramName] && param.enum && param.enum.indexOf(this.params[paramName]) === -1) {
        return done(new ResourceError(`Invalid resource parameter value '${this.params[paramName]}' for '${paramName}'`))
      }
    })
    if (!this.type) {
